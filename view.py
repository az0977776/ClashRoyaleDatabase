...
class Window(Frame):
    ...
    def init_window(self):
        ...
        self.login_frame.grid(row=0, column=0, rowspan=2, columnspan=1, sticky=W + E + N + S)
        ...
        self.ok.grid(row=3, column=3, sticky=E + W)
        ...
        self.read_frame.grid(row=1, column=3, rowspan=1, columnspan=3, sticky=W + E + N + S)
        ...
        self.write_frame.grid(row=3, column=0, rowspan=3, columnspan=3, sticky=W + E + N + S)
        for r in range(5):
        ...
        self.show_cards_frame.grid(row=0, column=6, rowspan=3, columnspan=3, sticky=W + E + N + S)
        for r in range(3):
        ...
        self.show_current_deck.grid(row=0, column=1)
        self.cur_deck = Entry(self.show_cards_frame, width=10)
        self.cur_deck.grid(row=0, column=0)

        # show all text
        self.show_all_text = Frame(self.master)
        self.show_all_text.grid(row=2, column=3, rowspan=3, columnspan=3, sticky=W + E + N + S)
        for r in range(3):
            self.show_all_text.rowconfigure(r, weight=1)
        for c in range(3):
            self.show_all_text.columnconfigure(c, weight=1)
        self.text = Text(self.show_all_text, height=10, width=53, state=DISABLED)
        self.text.grid(row=0, column=0)

        # show cards text
        self.show_cards_text = Frame(self.master)
        self.show_cards_text.grid(row=3, column=6, rowspan=2, columnspan=3, sticky=W + E + N + S)
        for r in range(2):
            self.show_cards_text.rowconfigure(r, weight=1)
        for c in range(3):
            self.show_cards_text.columnconfigure(c, weight=1)
        self.cards_in_deck = Text(self.show_cards_text, height=8, width=40, state=DISABLED)
        self.cards_in_deck.grid(row=0, column=0)

        #notif panel
        self.notif_frame = Frame(self.master)
        self.notif_frame.grid(row=0, column=3, rowspan=1, columnspan=3, sticky=W + E + N + S)
        for r in range(1):
            self.notif_frame.rowconfigure(r, weight=1)
        for c in range(3):
            self.notif_frame.columnconfigure(c, weight=1)
        self.show_notif = Text(self.notif_frame, height=1, width=52, state=DISABLED)
